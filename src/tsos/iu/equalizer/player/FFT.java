...
//Класс, который выполняет Быстрое преобразование Фурье.
public class FFT {
    ...
    void FFTAnalysis(final short[] input, int size) {
        ...
        while (i < n) {
            if (j > i) {
                Tmpr = Tmvl[i]; Tmvl[i] = Tmvl[j];Tmvl[j] = Tmpr; //dao cho tmvl(i) va  j
                Tmpr = Tmvl[i+1]; Tmvl[i+1] = Tmvl[j+1]; Tmvl[j+1] = Tmpr; //dao cho tmvl(i+1) va  j+1
